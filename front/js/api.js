...
class ApiManager {
	...
	async getFolderFileCount(folderId) {
		...
		try {
			...
			const [filesResponse, urlFilesResponse] = await Promise.all([
				fetch(`${this.baseUrl}/api/folders/${folderId}/count?user_id=${userId}`),
				fetch(`${this.baseUrl}/api/url-files/count?user_id=${userId}&folder_id=${folderId}`)
			]);
    ...
    async moveUrlFile(fileId, folderId) {
        ...
        try {
            const response = await fetch(`${this.baseUrl}/api/url-files/${fileId}/move?user_id=${userId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    async getUrlFiles(folderId = null) {
        ...
        let url = `${this.baseUrl}/api/url-files?user_id=${userId}`;
        if (folderId) {
            url += `&folder_id=${folderId}`;
        }
    ...
    async deleteUrlFile(fileId) {
        ...
        try {
            const response = await fetch(`${this.baseUrl}/api/url-files/${fileId}?user_id=${userId}`, {
                method: 'DELETE'
            });
    ...
    async createUrlFile(urlData) {
        ...
        try {
            const response = await fetch(this.buildApiUrl('/api/url-files'), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'User-UUID': userId
