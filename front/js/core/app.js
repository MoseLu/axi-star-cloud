...
class App {
     ...
     * 等待环境检测完成
     */
    async waitForEnvironmentDetection() {
        return new Promise((resolve) => {
            const checkEnvironmentDetection = () => {
                // 检查环境管理器是否已完成环境检测
                if (this.environmentManager && this.environmentManager.isEnvironmentDetected) {
                    resolve();
                } else {
                    setTimeout(checkEnvironmentDetection, 100);
                }
            };
