    ...
    updateFolderFileCountImmediately(folderId, increment = 1) {
        try {
            ...
            if (folderCard) {
                ...
                if (!countElement) {
                    // 尝试查找新格式的文件计数元素（在createFolderCardHTML中使用的格式）
                    // 查找包含"文件数："文本的容器，然后找到其中的数字span
                    const fileCountContainers = folderCard.querySelectorAll('.flex.items-center');
                    for (let container of fileCountContainers) {
